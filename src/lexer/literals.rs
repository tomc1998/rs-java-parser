...
impl LiteralsLexer {
    pub fn new() -> LiteralsLexer {
        LiteralsLexer {
            ...
            number_lit_regexes: [
                ...
                RegexBuilder::new(
                    r"^0x([0-9a-f]_?)+\.?([0-9a-f]_?)*(p(\+|-)\d+)?(f|d)?",
    ...
    pub fn lex<'a>(&self, input: &mut CharStream<'a>) -> Result<Option<Token<'a>>, &'static str> {
        ...
        if &input_str[..1] == "\"" {
            ...
            for re in self.number_lit_regexes.iter() {
...
mod tests {
    ...
    fn it_should_error_with_unterminated_string_literal() {
        ...
        let tok_0 = lexer.lex(&mut test_str);
        assert!(
            tok_0.is_err(),
    ...
    fn it_should_return_none_with_no_literal() {
        ...
        let tok_0 = lexer.lex(&mut test_str).unwrap();
